...
function getUserKey() {
    ...
    return "wrongQuestionBank_" + userName;
}
...
function startTimer(minutes) {
    ...
    timer = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            clearInterval(timer);
            alert("Đã hết thời gian. Bạn vẫn có thể nộp bài.");
        }
    }, 1000);
}
...
function submitQuiz() {
    ...
    document.getElementById("score").innerHTML = `
        <div class="result-summary">
            <p>📌 Tổng số câu: <b>${selectedQuestions.length}</b></p>
            <p>✅ Đúng: <b>${correct}</b></p>
            <p>❌ Sai: <b>${wrong}</b></p>
            <p>📊 Tỷ lệ đúng: <b>${percent.toFixed(2)}%</b></p>
            <p>⏱ Thời gian làm bài:
                <b>
    ...
    if (wrongQuestionsCurrentTest.length > 0) {
        ...
        document.getElementById("retryWrongBtn").innerHTML =
...
function renderReview() {
    ...
    selectedQuestions.forEach((q, index) => {
        ...
        q.options.forEach((opt, i) => {
            ...
            if (userAnswer === letter && letter !== q.answer) {
                cls = "wrong";
            }
...
function renderWrongStats() {
    const panel =
        document.getElementById("wrongStatsPanel");

    const data =
        JSON.parse(localStorage.getItem("wrongStats")) || {};

    const arr = Object.values(data);

    arr.sort((a, b) => b.count - a.count);

    let html = `<h2>🔥 Các câu sai thường gặp</h2>`;
        ...
        arr.forEach((item) => {
            ...
            if (item.options && Array.isArray(item.options)) {
                item.options.forEach((opt, i) => {
                    const letter = String.fromCharCode(65 + i);
                    const cls = letter === item.answer ? "correct" : "";
...
function updateWrongButton() {
    const count =
        Object.keys(
            JSON.parse(localStorage.getItem(getUserKey())) || {}
        ).length;

    const btn = document.getElementById("reviewWrongBtn");
    if (!btn) return;

    btn.innerHTML = `🔥 Ôn tập câu sai (${count})`;
}
...
document.getElementById("retryWrongBtn")?.addEventListener("click", () => {
    ...
    startTimer(Math.max(5, wrongQuestions.length));
});

document.getElementById("clearWrongStatsBtn")?.addEventListener("click", () => {
    if (confirm("Xóa toàn bộ thống kê câu sai?")) {
        localStorage.removeItem("wrongStats");
        renderWrongStats();
    }
});
